...
class CommentsEndpointTest : ProxerTest() {
    ...
    @Test
    fun testDefault() {
        server.enqueue(MockResponse().setBody(fromResource("comment_manga.json")))
    ...
    fun testEmptyRatingDetails() {
        server.enqueue(MockResponse().setBody(fromResource("comment_anime.json")))
    ...
    fun testEmptyRatingDetailsOther() {
        server.enqueue(MockResponse().setBody(fromResource("comment_anime_other.json")))
    ...
    fun testPath() {
        ...
        assertThat(server.takeRequest().path).isEqualTo("/api/v1/info/comments?id=1&p=0&limit=10&sort=rating")
    }

    private fun buildTestComment(): Comment {
        return Comment(
            "7502937", "6195", "293356", UserMediaProgress.WATCHING,
            RatingDetails(5, 5, 4, 5, 0),
            ...
            10, 84, 1, Date(1473258755L * 1000), "V.K.C.",
            "293356_5356c84799fed.jpg"
        )
    }

    private fun buildTestCommentWithEmptyRatingDetails(): Comment {
        return Comment(
            "20405741", "5840", "635753", UserMediaProgress.WATCHED,
            ...
            10, 0, 0, Date(1487696018L * 1000), "Hackl24", ""
        )
    }

    private fun buildTestCommentWithOtherEmptyRatingDetails(): Comment {
        return Comment(
