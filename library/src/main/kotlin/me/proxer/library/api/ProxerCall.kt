    ...
    private fun processResponse(response: Response<ProxerResponse<T>>): T? {
        if (response.isSuccessful) {
            ...
            if (proxerResponse.isSuccessful) {
                ...
                throw ProxerException(ProxerException.ErrorType.SERVER, proxerResponse.code, message)
            }
        } else {
            response.errorBody()?.also { it.close() }

            throw ProxerException(ProxerException.ErrorType.IO, message = "Unsuccessful request: ${response.code()}")
        }
    }

    private fun processNonProxerError(error: Exception): ProxerException {
        return when (error) {
            is SocketTimeoutException -> ProxerException(ProxerException.ErrorType.TIMEOUT, cause = error)
            is IOException -> when {
                error.message == "Canceled" -> ProxerException(ProxerException.ErrorType.CANCELLED, cause = error)
                else -> ProxerException(ProxerException.ErrorType.IO, cause = error)
            }
            is JsonDataException -> ProxerException(ProxerException.ErrorType.PARSING, cause = error)
            else -> ProxerException(ProxerException.ErrorType.UNKNOWN, cause = error)
        }
    }
}
