    ...
    private T processResponse(final Response<ProxerResponse<T>> response) throws ProxerException {
        if (response.isSuccessful()) {
            ...
            if (proxerResponse.isSuccessful()) {
                ...
                final StringBuilder message = new StringBuilder(proxerResponse.getMessage());

                if (proxerResponse.getData() != null) {
                    message.append('\n').append(proxerResponse.getData().toString());
                }
            ...
            throw new ProxerException(ProxerException.ErrorType.IO, (Integer) null,
                    "Unsuccessful request: " + response.code());
        }
    }
